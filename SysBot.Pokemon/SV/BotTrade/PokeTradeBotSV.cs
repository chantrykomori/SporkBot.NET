...
public class PokeTradeBotSV(PokeTradeHub<PK9> Hub, PokeBotState Config) : PokeRoutineExecutor9SV(Config), ICountBot
{
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleClone(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, byte[] oldEC, CancellationToken token)
    {
        ...
        if (!la.Valid)
        {
            Log($"Clone request (from {poke.Trainer.TrainerName}) has detected an invalid Pokémon: {GameInfo.GetStrings("eng").Species[offered.Species]}.");
            if (DumpSetting.Dump)
        ...
        poke.SendNotification(this, $"**Cloned your {GameInfo.GetStrings("eng").Species[clone.Species]}!**\nNow press B to cancel your offer and trade me a Pokémon you don't want.");
        Log($"Cloned a {GameInfo.GetStrings("eng").Species[clone.Species]}. Waiting for user to change their Pokémon...");
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleRandomLedy(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, PK9 toSend, PartnerDataHolder partner, CancellationToken token)
    {
        ...
        else if (config.LedyQuitIfNoMatch)
        {
            ...
            poke.SendNotification(this, $"No match found for the offered {GameInfo.GetStrings("eng").Species[offered.Species]}{nickname}.");
